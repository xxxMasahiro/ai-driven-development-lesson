...
Vite writes its mutable dependency cache under the repository's ignored Dashboard runtime root rather than under the configured application source root. The source/config inventory therefore remains immutable on both warm local workspaces and clean CI runners.
