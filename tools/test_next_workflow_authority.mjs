...
import { authorityFingerprint, composeAuthorityDecision, createSideEffectGateway, effectReceiptBindingFingerprint } from "./lib/next_workflow/authority.mjs";
...
test("missing, stale, denied, wrong-epoch, action, target, and approval sources fail closed", () => {
  ...
  const approval = { reason: "git_push_operation_approval", approval_id: "approval-1", target_id: "target", task_id: "task", authority_epoch: 2, fresh_until: "2030-01-01T00:00:00.000Z", proof_fingerprint: authorityFingerprint({ approval_id: "approval-1", signer: "developer-authority" }) };
...
function fixtureStore() {
  ...
  const store = openWorkflowStateStore({ repositoryRoot: root, expectedIdentity: { repository_logical_id: "repo", checkout_instance_id: "checkout" }, receiptProofVerifier: receiptStoreVerifier });
...
test("gateway requires an injected independent verifier and rejects forged receipt proof", async () => {
  const { store } = fixtureStore();
  ...
  await assert.rejects(() => gateway.execute({ variant: "git_effect", action: "push", subject: gitSubject(), bindings: bindings(), target: {}, request: {} }), /RECEIPT_VERIFIER_NOT_INDEPENDENT/);
  assert.equal(store.getIntent("effect-forged").state, "DISPATCHING");
  assert.equal(store.getOutbox({ intentId: "effect-forged" }).state, "sending");
  store.close();
});
...
test("gateway quarantines a claimed effect when authority is fenced immediately before dispatch", async () => {
  ...
  const fencedStore = new Proxy(store, {
    get(target, property) {
      if (property === "claimEffectDispatch") return (input) => {
        const claimed = target.claimEffectDispatch(input);
        target.fence({ reason: "revoked-after-dispatch-claim", expectedEpoch: input.authorityEpoch });
        return claimed;
      };
  ...
  const adapter = { async observe() { return { fingerprint: "same" }; }, async dispatch() { dispatched = true; }, matches() { return true; } };
  const gateway = createSideEffectGateway({ store: fencedStore, adapters: { git_effect: adapter }, sourceProvider: () => sources(), activationProvider: enforcedActivation, activationVerifier, currentCandidateProvider, reconciliationAuthorizer, receiptVerifier, clock: () => "2029-01-01T00:00:00.000Z", idFactory: () => "epoch-race" });
...
test("reconciliation cannot persist or reuse a receipt without independent proof", async () => {
  ...
  let proofMode = "allow";
  const gateway = createSideEffectGateway({ store, adapters: { git_effect: adapter }, sourceProvider: () => sources(), activationProvider: enforcedActivation, activationVerifier, currentCandidateProvider, reconciliationAuthorizer, receiptVerifier: (input) => {
    if (proofMode === "deny") return { verified: false };
    const proof = receiptVerifier(input);
    return proofMode === "wrong-binding" ? { ...proof, proof_record_id: `${proof.proof_record_id}-other` } : proof;
  }, clock: () => "2029-01-01T00:00:00.000Z", idFactory: () => "proof-check" });
  await assert.rejects(() => gateway.execute({ variant: "git_effect", action: "push", subject: gitSubject(), bindings: bindings(), target: {}, request: {} }), /SIDE_EFFECT_OUTCOME_UNKNOWN/);
  proofMode = "deny";
  await assert.rejects(() => gateway.reconcile("effect-proof-check"), /RECEIPT_INDEPENDENT_VERIFICATION_FAILED/);
  assert.equal(store.getReceipt("effect-proof-check"), undefined);
  proofMode = "allow";
  await gateway.reconcile("effect-proof-check");
  proofMode = "wrong-binding";
  await assert.rejects(() => gateway.reconcile("effect-proof-check"), /RECEIPT_STORED_PROOF_BINDING_INVALID/);
  proofMode = "deny";
  await assert.rejects(() => gateway.reconcile("effect-proof-check"), /RECEIPT_INDEPENDENT_VERIFICATION_FAILED/);
  store.close();
});
