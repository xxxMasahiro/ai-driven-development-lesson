...
function normalizeResultContract(value) {
  ...
  const contract = {
    ...
    finding_contract: {
      required_fields: ["code", "severity", "message", "evidence_refs"],
      code_pattern: "^[A-Za-z0-9][A-Za-z0-9_.:-]{0,63}$",
      code_format: "ASCII machine identifier only; never use whitespace, slash characters, paths, commands, or prose",
      code_examples: ["invariants_confirmed", "scope.blocked", "review:warning"],
      message_format: "single_line_plain_text_without_paths_commands_or_secrets",
      evidence_ref_pattern: "^[A-Za-z0-9][A-Za-z0-9_.:-]{0,127}$",
      no_findings_representation: [],
    },
