...
const CONTRACT_BASELINE_FINGERPRINTS = Object.freeze({
  ...
  next_workflow_team_agent_security: "26e3028385f735ddb479741fed2c25f2f472435151c8d4379fa1b60e1ebdaafe",
  next_workflow_provider_registry: "c133ed3c964fc023b32a05806a769c826e0c1abe765c41169aa1d5c386402ec3",
...
function validateTeam(value, issues) {
  ...
  requireCondition(issues, sameMembers(value.execution_preference?.modes, ["auto", "single_agent", "team"]) && value.execution_preference?.default === "auto" && value.execution_preference?.single_agent_uses_orchestrator_only === true && value.execution_preference?.single_agent_requires_effective_rigor === "L1" && value.execution_preference?.hard_l5_overrides_single_agent === true && value.execution_preference?.team_preference_may_raise_but_never_lower_rigor === true && value.execution_preference?.control_center_configuration === "deferred", "EXECUTION_PREFERENCE_INVALID", "single-Agent preference is L1-only, hard L5 overrides it, and a team preference cannot lower rigor", id);
...
function validateProviders(value, issues) {
  ...
  requireCondition(issues, value.attestation?.copied_or_predicted_actual_refused === true && value.attestation?.launch_observation_scope === "pinned_cli_launch_configuration" && value.attestation?.backend_attestation_available === false && value.attestation?.backend_identity_must_not_be_inferred_from_launch_arguments === true, "PLACEHOLDER_ACTUAL_ALLOWED", "predicted actual observation and backend inference from launch configuration must be refused", id);
  requireCondition(issues, value.automatic_effort_selection?.explicit_native_value_passed_to_cli === true && value.automatic_effort_selection?.selected_model_and_effort_bound_to_launch === true && value.automatic_effort_selection?.contained_process_argument_observation_required === true && value.automatic_effort_selection?.backend_attestation_not_claimed === true && value.automatic_effort_selection?.selected_actual_mismatch === "STOP" && value.automatic_effort_selection?.implicit_none_observation === "forbidden", "AUTOMATIC_EFFORT_SELECTION_INVALID", "automatic effort must be explicit and verified from contained launch arguments without claiming backend attestation", id);
